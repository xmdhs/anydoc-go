...
ANYDOC_REF="${ANYDOC_REF:-v0.2.4}"
GOCCY_REF="${GOCCY_REF:-f57f53361e9e507a8a8df83b82338c1901a77ce7}"
...
fetch() {
    ...
    # 不随本仓库提交）。anydoc 锁定已验证 tag（ANYDOC_REF）；goccy 固定到
    # 已验证 revision，确保生成器版本稳定。
...
apply_anydoc_patch() {
    for patch in "${ROOT}"/patches/*.patch; do
        ...
        apply_patch_once "${ANYDOC_DIR}" "${patch}" "${pname}"
    done
}
